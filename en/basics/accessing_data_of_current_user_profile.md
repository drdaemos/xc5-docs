...
Before we get started, we [create an empty module]({{ baseurl_lang }}/../getting_started/step_1_-_creating_simplest_module.md) with developer ID **Tony** and module ID **ProfileDemo**. Also, we [create a new page]({{ baseurl_lang }}/../basics/creating_new_page.md#Creatingnewpage-Creatingpageincustomerarea) that will be accessible via `cart.php?target=profile_demo` URL.
...
and the info about current account will be pulled into `$profile` variable that is an instance of `\XLite\Model\Profile` class (see [more info about classnames]({{ baseurl_lang }}/../misc/x-cart_classes_structure_and_namespaces.md) in X-Cart).
