...
We start with [creating an empty module]({{ baseurl_lang }}/../getting_started/step_1_-_creating_simplest_module.md) with developer ID **Tony** and module ID **ItemsListAdminDemo**. Then we [create a new page]({{ baseurl_lang }}/../basics/creating_new_page.md) `target=items_list_demo` in admin area. For that we create:
...
1.  Every single ItemsList in admin area extends`\XLite\View\ItemsList\Model\Table class`, so do we: 
    ...
    {% highlight php %}class ItemsListDemo extends \XLite\View\ItemsList\Model\Table{% endhighlight %}
2.  We need to point our **ItemsList** to some [model]({{ baseurl_lang }}/../basics/understanding_models.md). It will tell a widget what entities it must display. In our case, this entity is a **product**, so we point our ItemsList to `\XLite\Model\Product` model class: 
...
6.  Price column is a bit more complex also. Although, it has the same `COLUMN_NAME` and `COLUMN_ORDERBY` parameters, it supports sorting as defined by the `COLUMN_SORT` param. Aside from specifying this param in column, we must also create constant called `SORT_BY_MODE_PRICE` as it refers to it:
    ...
    {% highlight php %}const SORT_BY_MODE_PRICE = 'p.price';{% endhighlight %}
...
7.  Finally, we need to implement the `getSearchCondition()` method in our ItemsList: 
    ...
    {% highlight php %}    protected function getSearchCondition()
        {
            $result = parent::getSearchCondition();
    ...
    We need to extend it in order to allow [search() method]({{ baseurl_lang }}/../basics/searching_entities_in_repositories/search()_method.md) in the `\XLite\Model\Repo\Product` repository class to work properly.

8.  First, we let it know that sorting must be done according to user's selected condition: 

    {% highlight php %}$result->{\XLite\Model\Repo\Product::P_ORDER_BY} = $this->getOrderBy();{% endhighlight %}
...
9.  Then, we also add our condition of displaying products, which is it must display only product that are more expensive than $10: 
    ...
    {% highlight php %}$result->moreThan10 = true;{% endhighlight %}
10.  Finally, we need to add the `getSortByModeDefault()` method that will define a sorting option when it has not been chosen yet: 
    ...
    {% highlight php %}    protected function getSortByModeDefault()
        {
            return static::SORT_BY_MODE_PRICE;
        }{% endhighlight %}
...
{% highlight php %}<widget class="XLite\Module\Tony\ItemsListAdminDemo\View\ItemsList\ItemsListDemo" />{% endhighlight %}
