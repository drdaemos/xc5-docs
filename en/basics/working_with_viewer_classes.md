...
1.  [Create an empty module]({{ baseurl_lang }}/../getting_started/step_1_-_creating_simplest_module.md). I am creating it with developer ID **Tony** and module ID **ViewerDemo**.
...
4.  Let's walk through each line in order to understand what this code does:  
    ...
    `- @ListChild (list="order.operations", weight="150", zone="admin")` – definition of view list, where our content must be displayed. Have a look at [basic designer guide]({{ baseurl_lang }}/../getting_started/step_2_-_applying_design_changes.md#Step2-applyingdesignchanges-UnderstandingX-Cartlayoutsystem) in order to learn more about view lists and about how you can find the needed view list;  
...
8.  Now we need to apply several CSS rules in order to make it look smooth; check [adding CSS files]({{ baseurl_lang }}/../design_changes/adding_css_and_js_files.md) article for this task.
