...
First of all, you should have your custom module. We are [creating a new module]({{ baseurl_lang }}/developer_docs/getting_started/step_1_-_creating_simplest_module.html) with developer ID **Tony** and module ID **PopupDemo**.  
Secondly, we'll create a [custom page]({{ baseurl_lang }}/developer_docs/basics/creating_new_page.html) to have our popup button. This page will be available at `cart.php?target=tony_custom` address.
...
X-Cart has `\XLite\View\Button\APopupButton` class, which defines popup button. In order to use it, you should extend it with your custom button class (this thing is thoroughly explained in [Working with viewer classes]({{ baseurl_lang }}/developer_docs/basics/working_with_viewer_classes.html)). To do this, we are creating a file `<X-Cart>/classes/XLite/Module/Tony/PopupDemo/View/Button/DemoPopupButton.php` with the following content:
...
core.autoload(DemoPopupButton);{% endhighlight %}
...
This template uses several [Flexy]({{ baseurl_lang }}/developer_docs/basics/flexy_guide.html) features to show cart quantity, unordered list of items in the cart and subtotal in user currency. 
...
{% highlight php %}<widget class="\XLite\Module\Tony\PopupDemo\View\Button\DemoPopupButton" label="Demo popup!" />{% endhighlight %}
