...
Before we get started we have to understand what templates [render an invoice page]({{ baseurl_lang }}/developer_docs/getting_started/step_2_-_applying_design_changes.html). The responsible [viewer class]({{ baseurl_lang }}/developer_docs/basics/working_with_viewer_classes.html) for invoice display is `\XLite\View\Invoice` ([more info about classnames in X-Cart]({{ baseurl_lang }}/developer_docs/misc/x-cart_classes_structure_and_namespaces.html)) and as we can see in its code, it starts rendering the invoice from the `order/invoice/body.tpl` template (see `getDefaultTemplate()` method of the class) depending on the area:
...
2.  `image.tpl` template with the following content: 
    ...
    This code inserts an image wrapped into `<td></td>` element. This image is inserted via `\XLite\View\Image` widget as this widget performs [on-fly resizing routine]({{ baseurl_lang }}/developer_docs/basics/working_with_image_resizing_routine.html) according to `maxWidth` and `maxHeight` params. Again, since we insert this template with weight as **5**, it will be displayed at beginning of table's row.
