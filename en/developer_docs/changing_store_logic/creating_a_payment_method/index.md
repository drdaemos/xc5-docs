...
We start the creation of the payment gateway by [creating an empty module]({{ baseurl_lang }}/developer_docs/getting_started/step_1_-_creating_simplest_module.html) with the developer ID **Tony** and the module ID **PaymentDemo**. Then, we create the file  
...
Once the YAML file has been created, do not forget to [push it to the database]({{ baseurl_lang }}/developer_docs/getting_started/x-cart_sdk.html#X-CartSDK-LoadingYAMLfile).
