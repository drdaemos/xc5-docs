...
We start with [creating an empty module]({{ baseurl_lang }}/developer_docs/getting_started/step_1_-_creating_simplest_module.html) with developer ID **Tony** and module ID **ItemsListAdminDemo**. Then we [create a new page]({{ baseurl_lang }}/developer_docs/basics/creating_new_page.html) `target=items_list_demo` in admin area. For that we create:
...
2.  We need to point our **ItemsList** to some [model]({{ baseurl_lang }}/developer_docs/basics/understanding_models.html). It will tell a widget what entities it must display. In our case, this entity is a **product**, so we point our ItemsList to `\XLite\Model\Product` model class: 
...
7.  Finally, we need to implement the `getSearchCondition()` method in our ItemsList: 
    ...
    We need to extend it in order to allow [search() method]({{ baseurl_lang }}/developer_docs/basics/searching_entities_in_repositories/search()_method.html) in the `\XLite\Model\Repo\Product` repository class to work properly.
