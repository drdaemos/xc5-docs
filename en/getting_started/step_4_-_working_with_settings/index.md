...
*   {% link "Step 1 - creating simplest module" H1ar4zrP %}
*   {% link "Step 2 - applying design changes" Skx6SNzHP %}
*   {% link "Step 3 - applying logic changes" H1G6r4zSD %}
...
2.  Put the following content there: 
    ...
    {% endraw %}{% endhighlight %}
3.  Once it is done, push the changes to the database using {% link "load-yaml.php" S17mLEfHD#X-CartSDK-LoadingYAMLfile %} macros of X-Cart SDK. Since I am in the X-Cart's root folder (`<Web-Root>/next/src/`) in my console and my X-Cart SDK is located in the `<Web-Root>/next-sdk/`, I can do it using this command:
...
You can use all classes from the `<X-Cart>/classes/XLite/View/FormField/` folder in your settings or {% link "create your own one" r1-U4VMSv %}.
